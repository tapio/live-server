...
import MeldServer from "./index.mjs";
...
const configPath = path.join(homeDir, ".meld-server.json")
...
for (let i = process.argv.length - 1; i >= 2; --i) {
  ...
  // ... (Other arguments processed similarly)
    ...
    setTimeout(MeldServer.shutdown, 500)
    process.argv.splice(i, 1)
    ...
    console.log(
      "Usage: meld-server [-v|--version] [-h|--help] [-q|--quiet] [--port=PORT] [--host=HOST] [--open=PATH] [--no-browser] [--browser=BROWSER] [--ignore=PATH] [--ignorePattern=RGXP] [--no-css-inject] [--entry-file=PATH] [--spa] [--mount=ROUTE:PATH] [--wait=MILLISECONDS] [--htpasswd=PATH] [--cors] [--https=PATH] [--https-module=MODULE_NAME] [--proxy=PATH] [PATH]",
    )
    process.exit()
  }
